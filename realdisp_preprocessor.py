...
def load_set(window_size, overlay):
    ...
    for i in range(1,18):
        ...
        for j in range(0, 6):
            ...
            path = 'REALDISP/realistic_sensor_displacement/subject' + str(i) + '_' + file_names[j] + '.log'
...
def merge_subjects(window_size):
    ...
    for i in range(1, 18):
        if i == 1:
            ...
            data = np.concatenate((data, pickle.load(open(path + 'Subject' + str(i) + '_preprocessed.txt' ,'rb'))), axis =0)
